...
> Updated: 2026-06-23
> Purpose: Hold Siesta event work and tighten the current product logic around Karua, recommendation, serving flow, and session closure. Some of these decisions have been updated since writing.

## Current Scope

Siesta is still preserved as a low-frequency banter event, but runtime Siesta banter remains disabled for now while the Karua-only flow is stabilized. The character structure, event design, and sprite assets are preserved as parked assets for future re-enabling.
...
The product loop is judged in this order:
