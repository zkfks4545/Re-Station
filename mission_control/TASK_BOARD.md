...
| PIPE-806 FSM 점진 적용 | DOING | A `f691adf`: safety·추천 취소 ControlIntent transition 소비 완료. 다음 B topic·speech·entity → C preference → D Conversation Expansion | A: 65 files / 861 tests, check, lint, build, safety·cancel Replay와 불일치 fallback | A 완료. 전체 완료 조건은 단일 intent 의존 제거, 각 소비 단계 독립 검증, P0.5 계약 유지 |
