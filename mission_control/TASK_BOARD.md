...
| RST-409 | 시에스타 대사 풀 확장 및 다양성 개선 | DONE | 현재 구현 기준 7개 브랜치·21개 대사 세트, 키 기반 중복 방지, `celebration`/`sweet`/`sad` 브랜치 포함. Vitest 97개 통과 |
...
| PIPE-802 WebLLM 제거 | DONE | App/controller 연결, hook/component, `src/lib/webllm`, 환경 변수, `@mlc-ai/web-llm` 제거 | 62 files / 823 tests, check, lint, build, 대표 20 tests, source/package/asset 무참조 검사 | 구현 `0b29a0e`. main 543.44 kB(gzip 162.32 kB), WebLLM worker/lib 청크 제거, 사용자 동작 불변 |
| PIPE-803 Replay 기반 | DONE | 기존 continuity·regression 경계를 정규화 snapshot, 정적 corpus, severity diff runner로 재사용 | 63 files / 827 tests, check, lint, build. 실제 DialogueService 반복 실행·safety·farewell 추천 차단·severity 계약 | 구현 `d5d086c`. critical/major 자동 실패, review/allowed 분리, 운영 원문 저장·새 소비자 없음 |
| PIPE-804 Understand 계약 | DONE | `TurnInput`, `InputUnderstanding` 다중 신호, 턴 단위 `ConversationStateCue`, evidence/confidence/span, `SemanticAssistProposal` validator와 fake/no-op adapter | 64 files / 843 tests, check, lint, build. classifier fixture shadow, 복합 발화, 호출 자격, 금지 필드·invalid span | 구현 `daae94c`. 런타임 소비자 없음, 모든 신호 근거 보유, API는 검증된 의미 후보만 표현 가능 |
| PIPE-805 Evaluate·Select·Plan shadow | DONE | 칵테일·질문·대화별 legacy `Evaluation`, 결정적 `Selection`, `DialogueMove`, `PreferenceEvidence` 원장과 기존 상태 projection | 65 files / 859 tests, check, lint, build. 동점·hard constraint·무변경·legacy 선택 일치·action/move·scope 검증 | 구현 `a6129f7`. Plan만 transition 생성, 기존 추천·FSM 소비와 결과 불변 |
| PIPE-806 FSM 점진 적용 | DONE | A `f691adf`: ControlIntent. B `507abfc`: topic·speech·entity. B.1 `ea3527c`: active session. B.2 `a02abe8`: 카루아 취향. C `19d032c`: PreferenceEvidence. D `7a7c088`: Conversation Expansion Plan 소비 | D: 67 files / 914 tests, check, lint, build, 중단 topic·Plan 호환 Replay·legacy fallback·동일 질문 복귀 | controller가 다중 신호와 Plan을 단계별 소비하며, 각 단계 독립 gate·fallback과 P0.5 계약 유지 |
| PIPE-807 평가 근거 통합 | DONE | 공통 `CandidateEvaluation<T>`; 칵테일 hard constraint·taste score·근거 contribution, 질문 분리도·goal 문맥·답변 난이도·피로도 contribution | 120개 조합 기존 선택 일치, 최대 3문항·known/asked 회피, 결정성, 점수 합, 이유-evidence 동일성, 67 files / 918 tests | 구현 `3529cff`. 추천 이유는 선택 evaluation contribution에서만 파생 |
...
- 시에스타 만담 속 카루아 발화는 최종 감사 범위에 포함했다. 다만 2026-07-20 재검수에서 `SIESTA_EVENTS_ENABLED = false`가 확인되어 실제 런타임은 비활성 상태로 정정한다. 엔진·대사 데이터는 보존되어 있으며 재활성화 조건은 `SIESTA.md`가 소유한다.
