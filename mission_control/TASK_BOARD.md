...
| PIPE-806 FSM 점진 적용 | DOING | A `f691adf`: ControlIntent 완료. B `507abfc`: topic·speech·entity 완료. B.1 `ea3527c`: story→recommendation active session 원자 전환·stale 입력 거부 완료. 다음 C PreferenceEvidence → D Conversation Expansion | B.1: 65 files / 868 tests, check, lint, build, 세션 소유권·선택 payload·잡담 복귀 회귀 | A/B/B.1 완료. 전체 완료 조건은 단일 intent 의존 제거, 각 소비 단계 독립 검증, P0.5 계약 유지 |
