...
| DEC-026 | 07-02 | 승인 | WebLLM 준비 기본 ON, 접속 직후 capability 통과 시 자동 다운로드, 응답 기본 OFF | 현재 작업 |
...
DEC-025의 준비 기본값만 대체한다. PRELOAD는 별도 설정이 없어도 활성화하며 접속 직후 capability 검사를 통과하면 Web Worker에서 모델 준비를 시작한다. 응답 생성은 계속 기본 OFF이고 실제 대화 출력 연결 여부와 독립적으로 관리한다.
