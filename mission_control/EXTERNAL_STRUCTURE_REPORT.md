...
* **독립 드라이함 축 (`dryness`):** `DEC-026`에 따라 드라이함은 단맛의 반대 파생값이 아닌 독립 DB feature로 분리되어 관리됩니다. 달지 않지만 드라이하지 않거나, 달콤하면서도 드라이한 피니시를 가진 칵테일의 복합적인 표현이 가능합니다.
* **표시용 반별점 스케일 (`DEC-025`):** 추천 및 칵테일 카드, 도감에 노출되는 맛 프로필(단맛, 신맛, 드라이함, 도수)은 Normalized 값을 1~5 범위의 **0.5 반별점 단위**로 매핑하여 표시합니다.
