...
차기 세션 방향성은 `mission_control/SESSION_FLOW_SPEC.md`를 기준으로 한다. 환상주점은 AI 챗봇이나 연애 미연시가 아니며, 자유입력은 허용하되 세션 진행은 웰컴드링크, 추천, 주문, XYZ, 배웅, 귀가로 닫힌 구조를 따른다.
...
차기 구조에서는 추천 카드 표시 이후에도 세션이 무한 대화로 열려 있지 않아야 한다.
...
XYZ: 오늘의 마지막 드링크
  ↓
Farewell Phase: 2~3턴 배웅 대화
  ↓
귀가
```

XYZ 이후에는 추가 주문과 신규 추천이 금지된다. 사용자는 XYZ 설명, 칵테일 유래, 후기, 오늘 추천 결과, 가벼운 잡담, 다음 방문 암시 정도는 말할 수 있지만 추천 루프로 재진입하지 않는다.

세션 분위기 상태값은 `trust`, `familiarity`, `playfulness`, `tension` 같은 말투 조절 신호로만 사용한다. 호감도, 연애 루트, 엔딩 분기, 사용자 평가 용도로 사용하지 않는다.
...
4. `FLOW-002` XYZ와 Farewell Phase 상태 머신 설계
5. `FLOW-003` 선택지 이벤트와 자유입력 복귀 정책 구현
6. `SPR-001` 캐릭터 스프라이트 슬롯 계약
7. `SPR-002` 카루아 표정별 스프라이트 연결
8. `SPR-003` 시에스타 난입 스프라이트 표시
9. `SPR-004` 시에스타 이벤트 스프라이트 큐 연결
10. `SPR-005` 캐릭터 에셋 제작·정리 가이드
