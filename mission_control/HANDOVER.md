...
1. `CURRENT_STATE.md`의 현재 우선순위와 `TASK_BOARD.md`의 해당 작업 범위를 맞춰 본다.
2. 코드 작업을 시작하기 전에 관련 도메인 문서만 추가로 연다.
3. 변경 범위를 정한 뒤 기존 테스트 계약을 먼저 확인한다.
4. 작업 후 `WORK_LOG.md`, `CURRENT_STATE.md`, 필요 시 `TASK_BOARD.md`만 갱신한다.
5. 다음 작업자가 알아야 할 실행상 주의가 생겼을 때만 이 문서를 갱신한다.
...
- WebLLM 의미 태그는 검증된 낮은 우선순위 힌트일 뿐이며 규칙 기반 흐름을 대체하지 않는다.
