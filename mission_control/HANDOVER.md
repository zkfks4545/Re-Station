...
- [x] **FLOW-002 XYZ와 Farewell Phase 상태 머신**: 무한 루프 챗봇화를 막기 위해 세션 단계와 마지막 잔 정책을 런타임에 연결했다. 웰컴드링크는 누적 도수 계산에서 제외하고, 일반 주문/추천으로 마신 칵테일의 누적 도수 별점이 10을 넘은 뒤 다음 주문·추천 요청은 XYZ로 전환한다. XYZ 이후에는 신규 주문·추천과 다시 추천받기를 차단하고 Farewell Phase 3턴 뒤 귀가로 닫는다. Vitest 160개, lint, build가 통과한다.
