...
### 2026-06-29 — Claude — kf 중복 제거 + 미사용 export 정리 + MOOD_KEYWORD_MAP 데이터화 + 테스트 292→308

**코드 정리:**
- `kf()` 유틸 `pattern-utils.ts` 공유 모듈로 추출 (intent-classifier, conversation, question-engine 3개 중복 제거)
- 미사용 export 제거: `detectCocktailInfoQuery`, `detectOrderVerb`, `DialogueAction`(action-resolver)
- `MOOD_KEYWORD_MAP` `response-templates.ts`에 데이터화, `detectUserMood` map 기반으로 전환
- intent classifier mood-talk 키워드 동기화 (`"피곤"`, `"졸리"` 추가)

**SHAKE_REFERENCE 3파일 중복 제거:**
- `pattern-utils.ts` 공용 `SHAKE_REFERENCE` 상수로 통일 (input-router, conversation, useRestationController)

**switch 패턴 헬퍼 추출:**
- `pickStoryFallback()` — STORY_FALLBACK lookup (conversation.ts 2개 중복 블록)
- `resolveFromSubTemplate(tmpl, defaultTmpl, extraDefaultCheck?)` — mood/taste/rude 3개 case 통일

**테스트:**
- 292→313 tests (+21): kf edge case 4, SHAKE_REFERENCE 5, MOOD_KEYWORD_MAP 정합성 4, mood 통합 2, SHAKE 라우팅 3
