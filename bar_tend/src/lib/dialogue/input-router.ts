...
const COCKTAIL_INFO_QUERY = /정보\s*(?:좀\s*)?(?:알려|줘|뭐야|뭔지)?|레시피\s*(?:좀\s*)?(?:알려|줘|뭐)?|재료(?:는|가|은|이)?\s*(?:뭐|무슨|어떻|좀|알려)|도수\s*(?:는|가)?\s*(?:어떻|뭐|얼마|높|낮)|맛(?:은|이|은요|이요)?\s*(?:설명|어때|어떻|뭐|알려)|어떤\s*칵테일|이\s*칵테일\s*(?:정보|설명|레시피)|(?:설명|뭐야|뭔데)\s*[?.!]*$/
